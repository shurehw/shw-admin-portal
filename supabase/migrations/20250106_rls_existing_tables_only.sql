-- Row Level Security for Existing Tables Only
-- ============================================
...
-- Enable RLS on the association tables we just created
ALTER TABLE company_contacts ENABLE ROW LEVEL SECURITY;
ALTER TABLE deal_contacts ENABLE ROW LEVEL SECURITY;
ALTER TABLE deal_companies ENABLE ROW LEVEL SECURITY;

-- Simple policies for core tables
-- Companies
CREATE POLICY "Authenticated users can view companies" ON companies
  FOR SELECT USING (auth.uid() IS NOT NULL AND (deleted_at IS NULL OR deleted_at IS NULL));
...
-- Success message
SELECT 'RLS policies applied to existing tables' as status;