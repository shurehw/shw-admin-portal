...
import { 
  ...
  RefreshCw, Eye, User, Calendar, ArrowRight
} from 'lucide-react';
...
export default function CompanyTicketsCard({ companyId, primaryContactId }: CompanyTicketsCardProps) {
  ...
  const loadTicketData = async (force = false) => {
    try {
      if (force) {
        setRefreshing(true);
        crmTicketingAPI.invalidateCompanyCache(companyId);
      } else {
        setLoading(true);
      }
      ...
      const [summaryData, ticketsData] = await Promise.all([
        crmTicketingAPI.getCompanyTicketSummary(companyId),
        crmTicketingAPI.getCompanyTickets(companyId, 5)
      ]);
  ...
  const handleOpenTicketing = () => {
    const url = crmTicketingAPI.generateTicketingDashboardLink({ companyId });
    window.open(url, '_blank');
  };
