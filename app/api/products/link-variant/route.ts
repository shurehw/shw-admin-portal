...
export async function POST(request: Request) {
  try {
    ...
    // Add variant to parent's variant array
    const updatedVariants = parent.product_variants || []
    if (!updatedVariants.includes(variantId)) {
      updatedVariants.push(variantId)
    }
    
    // Update parent with new variant
    ...
    return NextResponse.json({ error: 'Failed to link variant' }, { status: 500 })
  }
}